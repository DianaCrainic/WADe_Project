...
const getCryptoNewsData = async(currency: string) : Promise<CryptoNews[]> => {
    let collectedCryptoNews: CryptoNews[] = [];
    let currentPage = 1;
    while(true) {
        if (currentPage > 3) {
            break;
        ...
        try {
            const { data } = await axios.get<any>(`${MESSARI_API_URL}${currency}`, {
                params: {
                    page: currentPage
                },
                headers :{
            ...
            if(data.data) {
                ...
                for(let dataIndex = 0; dataIndex < data.data.length; dataIndex++) {
                    ...
                    let cryptoNewsEntry: CryptoNews = {
                        id: `http://schema.org/${crypto.randomUUID()}`,
...
const main = async () => {
    const currencyName = "zcash"; // use as currency name one of the keys from currenciesObject
